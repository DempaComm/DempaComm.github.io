...
class SiteOutputContractTest(unittest.TestCase):
    ...
    def test_exploration_pages_and_machine_readable_data_are_generated(self) -> None:
        ...
        normal_node = next(node for node in graph["nodes"] if node["title"] == "通常原稿")
        self.assertEqual("", normal_node["html_path"])
        self.assertEqual(0, normal_node["statement_count"])
        self.assertEqual(0, normal_node["correction_count"])
