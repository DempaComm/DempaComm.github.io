...
1. 指定ファイルを `papers/<slug>/` へコピーする。
2. コピー元とコピー先のSHA-256が一致することを確認する。
3. 原本SHA-256と現在承認済みSHA-256が同一の `paper.json` を作る。
4. 標準の `.latexmkrc` がなければ生成する。
5. `paper.json` から `index.html` の原稿一覧を再生成する。
...
したがって、新しい `paper.json` を追加すれば、記事一覧、PDF生成対象、Pagesへの配置へ自動的に反映される。
