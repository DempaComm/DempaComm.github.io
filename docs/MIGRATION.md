...
原稿情報、サイト実装、手順書などを変更した後は、リポジトリのルートで次の一つを
実行する。
...
Pagefindの初回導入だけは `docs/PAGEFIND.md` の準備を行う。
...
```sh
python3 scripts/paper_tool.py check-all --output preview-site
```
...
追加機能は `dempa_site/features/registry.py` で一括登録し、無効化した機能は実行しない。
新しい生成機能の契約、登録方法、失敗時の規則は `docs/SITE_FEATURES.md` に記載している。
...
`paper.json` の `math_section` は数学記事総覧での主分類を表す。空欄または省略時は「その他」として扱う。公開時に `math/` へ分野別総覧の総合入口を生成し、`math/<分野>/` へ各分野の記事を公開年別・タグ付きで自動生成する。さらに、`dempa_site/config.py` の `MATH_TOPICS` と電波通信タグから、重複を許すテーマ別総覧 `math/topics/<テーマ>/` を生成する。テーマ追加のために各 `paper.json` を書き換える必要はない。トップページと「数学」タグページから総合入口へ案内する。
