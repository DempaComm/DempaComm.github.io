...
題名保持の検査では、ブログ記事名とTeX内部の正式題名が異なる場合があるため文字列の完全一致は
要求しない。LaTeXMLが文書題名用の見出し要素を生成し、その内容が空でないことを確認する。
`Untitled Document` しかないPDF貼り込み用ラッパーなどは題名保持とみなさない。
...
個別の手動承認版として扱う場合は、この条件を満たしても、数式、定理番号、相互参照、内容を
元PDFと目視比較してから公開する。

## 全TeX原稿を一括変換する

サイト全体の変換状況を先に把握したい場合は、一括モードを使う。

```sh
python3 scripts/paper_tool.py latexml-batch \
  --output _experiments/latexml-all-YYYY-MM-DD
```

このモードはHTML版が未登録の全TeX原稿を順番に変換し、既存HTML版とTeXのない記事は
自動的に除外する。各原稿の失敗で全体を止めず、結果を出力先の `report.json` にまとめる。
警告・エラー、`ltx_ERROR`、題名欠落、図版欠落、個人情報候補、危険なHTMLまたはSVGが
一つもない自動検査合格分だけを公開フォルダへ移す。

一括公開されたHTMLは「自動変換・未目視」と明示し、手動承認済みのHTMLとは区別する。
元PDF・TeXが正本であることに変わりはない。不合格原稿はサイトへ追加されず、
`blocking_reasons` とログを使ってデプロイ後に一件ずつ直す。自動合格分も後日PDFと比較し、
確認できたものから通常の手動承認相当へ更新する。
...
保持された。目視確認済みの正式版は `papers/2024-01-08-01/html/index.html` として
公開している。公開承認時にはこのHTMLと元PDFを人が比較した。
...
目次、394件の内部参照、3点のXy-pic図、58件の文献も生成された。目視確認済みの正式版は
`papers/2018-10-14-01/html/index.html` として公開している。
...
独自BSTの外観は再現せず、LaTeXMLの意味的な標準書式を使う。目視確認済みの正式版は
`papers/2024-01-13-01/html/index.html` として公開している。
...
簡易個人情報候補がすべて0となった。目視確認済みの正式版は
`papers/2021-05-22-01/html/index.html` として公開している。

## 一括変換後の頻出警告への対応

全TeX原稿の一括変換後は、警告を出現数順に集計して変換処理側の共通原因を修正した。
元TeX・PDFは変更していない。

- 左上添字による関数空間表記 `{}^{A}B` を、変換用コピーだけで同値な `B^{A}` へ並べ替える。
- `\|x\|`、左右とも `\lVert` のノルム、行全体で左右が逆転したノルムを
  `\lVert x\rVert` へ正規化する。
- 開き括弧に右用命令を使った `\Bigr(` を `\Bigl(` へ直す。
- 歴史的原稿が読み込む `fbox` について、標準LaTeXの箱命令をそのまま使う空の互換bindingを置く。

関数空間表記が集中する原稿では警告が69件から1件まで減った。残った括弧欠落や
`f:\to Y` のような定義域欠落は、変換器の互換問題ではなく元TeXの記述に関わるため、
数学的内容を推測して自動修正しない。個別確認対象としてレポートに残す。
