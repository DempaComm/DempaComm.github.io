...
移行は完了しており、台帳の実装は監査・保守用として
`tools/legacy_migration/` に隔離している。操作方法は変更していないため、この文書の
コマンドはすべて従来どおり `scripts/migration_ledger.py` から実行する。通常のサイト
生成は台帳CSV・JSONや候補照合処理に依存しない。
...
通常走査の対象外にある原稿をMyBlog全体の調査で手動確認した場合は、`source_dir` を
`MyBlog/` で始め、作業フォルダからの相対表記（例: `MyBlog/2026/example`）で記録します。
この場合も絶対パスは記録しません。`tex_files`、`pdf_files`、`bib_files`、`bst_files`
には、その候補から移行対象にするファイルだけを書き、同じフォルダにある第三者資料や
旧版は含めません。状態を `source_found` にすると、MTエクスポートを再同期しても候補
情報は保持されます。
...
- 「僕のお気に入り」タグ付きで、未移行・未公開記事・複製ではない候補を初期表示
- 照合区分、公開年、判定状態、文字列による絞り込み
- 公開済み記事の別版、複製候補、全タグ表示への切り替え
