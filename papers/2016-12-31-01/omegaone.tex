...
\[
A\cap \biggl((\al,\to)\times (\be,\to)\biggr)\neq \O
\]
...
\[
A\cap \biggl((\ga,\to)\times (\ga,\to)\biggr)=\O
\]
を充たすものとする。さて$\omega_1\times \omega_1$は次のように直和分解できる。
\[
\omega_1\times \omega_1=\biggl([0,\ga]\times[0,\ga]\biggr)\cup \biggl([0,\ga]\times(\ga,\to)\biggr)\cup \biggl((\ga,\to)\times [0,\ga]\biggr)\cup \biggl((\ga,\to)\times (\ga,\to)\biggr)
\]
...
\end{align*}
ここで$L=\{x\in \omega_1\mid \text{$x$は極限順序数}\}$である。このように定義すると$A,B$はともに$S$の閉集合であり、$A\cap B=\O$である。ここで$A$を含む任意の$S$の開集合$U$を取る。$\bigl\{(\al,x]\times \{x+1\}\mid \al<x\bigr\}$は$(x,x+1)$の基本近傍系を成すので各$x\in L$ごとに$f(x)$を選んで
\[
