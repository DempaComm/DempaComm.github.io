...
であるので
\[
|F^{(1)}(t)|\le L\lVert x-y\rVert\left(\sum_{i=1}^m\left|\frac{\partial f}{\partial x_i}(\phi(y+t(x-y))) \right|\right)
\]
となる。
さて仮定から
\[
\left|\frac{\partial f}{\partial x_i}(\phi(y+t(x-y))) \right|\le b(\lVert y+t(x-y)-y\rVert)\lVert y+t(x-y)-y\rVert^{k-1}
\]
が成り立ち、$t\in (0,1)$なので
\[
\left|\frac{\partial f}{\partial x_i}(\phi(y+t(x-y))) \right|\le b(\lVert x-y\rVert)\lVert x-y\rVert^{k-1}
\]
となる。
よって$K=mL$と置けば
\[
