...
そして$B^*$上のノルム$\lVert\cdot\rVert_{B^*}$は
\[
\|f\|_{B^*}=\sup_{x\in B, x\neq 0}\frac{|f(x)|}{\|x\|_B}=
\sup_{\|x\|_B\le 1, x\neq 0}\frac{|f(x)|}{\|x\|_B}
=
...
一般にバナッハ空間$B$のノルムは単に$\lVert\cdot\rVert$と書くが，区別の必要があるときには$\lVert\cdot\rVert_B$と書く．
...
$x\in B$で添字付けられた写像族$\left.\tau_x\right|_S\colon S\to \rr$の元の全てを連続にする$S$上の位相のことを
\textbf{$S$上の汎弱位相}と呼ぶ．
...
\]
で$C_b(X)$上のノルム$\lVert\cdot\rVert_{C(X)}$
を定義する．
このノルムを単に$\lVert\cdot\rVert$と省略したりする．
