...
バージョン `0.1.0a0` の実験段階である。現時点の補正器は、文字列・コメント外に残った
`識別子\neq識別子` をTypstの `識別子 != 識別子` へ変換する。定理環境、ラベル、参照、
未対応LaTeX命令を検出した場合は、推測で直さず停止する。
